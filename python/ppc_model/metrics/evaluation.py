...
class Evaluation:
        ...
        if test_praba is not None:
            ...
            if train_praba is not None:
                ...
                if dataset.train_y is not None:
                    self.summary_evaluation(dataset, test_ks, test_auc, train_ks, train_auc)
    ...
    def calculate_ks_and_stats(predicted_proba, actual_label, num_buckets=10):
        ...
        df = pd.DataFrame({'predicted_proba': predicted_proba.reshape(-1), 'actual_label': actual_label.reshape(-1)})
        # 根据预测概率降序排列
        df_sorted = df.sort_values(by='predicted_proba', ascending=False)
        # 将数据划分为 num_buckets 个分组
        try:
            df_sorted['bucket'] = pd.qcut(df_sorted['predicted_proba'], num_buckets, retbins=True, labels=False)[0]
        except Exception:
            df_sorted['bucket'] = pd.cut(df_sorted['predicted_proba'], num_buckets, retbins=True, labels=False)[0]
        # 统计每个分组的信息
        ...
        stats['cum_positive_ratio'] = stats['positive_count'].iloc[::-1].cumsum()[::-1] / stats['positive_count'].sum()
        stats = stats[['count_ratio', 'count', 'positive_count',
                    'positive_ratio', 'negative_ratio', 'cum_positive_ratio']].reset_index()
        stats.columns = ['分组', '样本占比', '样本数', '正样本数', '正样本比例', '负样本比例', '累积正样本占比']
        return stats
    ...
    def evaluation_file(self, ctx, data_index: np.ndarray, 
        ...
        if y_true is not None:
            ...
            while retry_num < max_retry:
                ...
                try:
                    with plot_lock:
                        ks_value, auc_value = Evaluation.plot_two_class_graph(self, y_true, y_praba)
                except:
                    ctx.components.logger().info(f'y_true = {len(y_true)}, {y_true[0:2]}')
                    ctx.components.logger().info(f'y_praba = {len(y_praba)}, {y_praba[0:2]}')
            ...
            ResultFileHandling._upload_file(self.storage_client, self.metric_roc_file, self.remote_metric_roc_file)
            ResultFileHandling._upload_file(self.storage_client, self.metric_ks_file, self.remote_metric_ks_file)
            ResultFileHandling._upload_file(self.storage_client, self.metric_pr_file, self.remote_metric_pr_file)
            ResultFileHandling._upload_file(self.storage_client, self.metric_acc_file, self.remote_metric_acc_file)

            # ks table
            ks_table = self.calculate_ks_and_stats(y_praba, y_true)
            ks_table.to_csv(self.metric_ks_table, header=True, index=None)
            ResultFileHandling._upload_file(self.storage_client, self.metric_ks_table, self.remote_metric_ks_table)
        else:
            ks_value = auc_value = None
        
        # predict result
        self._parse_model_result(data_index, y_true, y_praba)
        ResultFileHandling._upload_file(self.storage_client, self.model_output_file, self.remote_model_output_file)
    ...
    def _parse_model_result(self, data_index, y_true=None, y_praba=None):
        ...
        if y_true is None:
            df = pd.DataFrame(np.column_stack((data_index, y_praba)), columns=['id', 'class_pred'])
        else:
