...
#include "ppc-crypto/src/hash/HashFactoryImpl.h"
...
BOOST_AUTO_TEST_CASE(testBaseOT)
{
    ...
    // for(auto iter = messageKeypair.begin(); iter != messageKeypair.end(); ++iter)
    // {
    ...
    auto senderMessage = baseOT->senderGenerateCipher(bcos::bytes(choice.begin(), choice.end()), obfuscatedOrder);
    // std::cout<< "baseOT->receiverGenerateMessage" << std::endl;
    auto receiverMessage = baseOT->receiverGenerateMessage(senderMessage.pointX, senderMessage.pointY, messageKeypair, senderMessage.pointZ);

    // std::cout<< "baseOT->finishSender" << std::endl;
    auto result = baseOT->finishSender(senderMessage.scalarBlidingB, receiverMessage.pointWList, receiverMessage.encryptMessagePair, receiverMessage.encryptCipher);
...
void testOTPIR(FakeOtPIRFactory::Ptr _factory, OtPIRImpl::Ptr _sender,
    OtPIRImpl::Ptr _receiver, ppc::protocol::Task::ConstPtr _senderPirTask,
    ...
    int _expectedErrorCode = 0)
{
    ...
    // {
    //     checkTaskPIRResult(_factory->resourceLoader(), _receiverPirTask, _expectedPIRResult.size(),
    //         _expectedPIRResult);
    // }
}
...
BOOST_AUTO_TEST_CASE(testNormalOtPIRCase)
{
    ...
    std::string jobParams = "{\"searchId\":\"testmsg1100\",\"requestAgencyId\":\"receiver\",\"prefixLength\":6}";
